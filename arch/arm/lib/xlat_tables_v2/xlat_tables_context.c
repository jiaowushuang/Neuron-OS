		     ...
		     unsigned int attr)
{
	...
	mmap_add_region_ctx(&xxx_xlat_ctx, &mm);
}

void mmap_add(const mmap_region_t *mm)
{
	mmap_add_ctx(&xxx_xlat_ctx, mm);
}
			      ...
			      size_t size, unsigned int attr)
{
	...
	mmap_add_region_alloc_va_ctx(&xxx_xlat_ctx, &mm);
			    ...
			    size_t size, unsigned int attr)
{
	...
	return mmap_add_dynamic_region_ctx(&xxx_xlat_ctx, &mm);
}
				     ...
				     unsigned int attr)
{
	...
	int rc = mmap_add_dynamic_region_alloc_va_ctx(&xxx_xlat_ctx, &mm);
...
{
	return mmap_remove_dynamic_region_ctx(&xxx_xlat_ctx,
					base_va, size);
}
...
{
	assert(xxx_xlat_ctx.xlat_regime == EL_REGIME_INVALID);

	unsigned int current_el = xlat_arch_current_el();

	if (current_el == 1U) {
		xxx_xlat_ctx.xlat_regime = EL1_EL0_REGIME;
	} else if (current_el == 2U) {
		xxx_xlat_ctx.xlat_regime = EL2_REGIME;
	} else {
		assert(current_el == 3U);
		xxx_xlat_ctx.xlat_regime = EL3_REGIME;
	}

	init_xlat_tables_ctx(&xxx_xlat_ctx);
}

int xlat_get_mem_attributes(uintptr_t base_va, uint32_t *attr)
{
	return xlat_get_mem_attributes_ctx(&xxx_xlat_ctx, base_va, attr);
}

int xlat_change_mem_attributes(uintptr_t base_va, size_t size, uint32_t attr)
{
	return xlat_change_mem_attributes_ctx(&xxx_xlat_ctx, base_va, size, attr);
}
...
{
	assert(xxx_xlat_ctx.initialized == true);
#ifdef __aarch64__
	if (xxx_xlat_ctx.xlat_regime == EL1_EL0_REGIME) {
		disable_mmu_el1();
	} else if (xxx_xlat_ctx.xlat_regime == EL3_REGIME) {
		disable_mmu_el3();
	} else {
		assert(xxx_xlat_ctx.xlat_regime == EL2_REGIME);
		return -1;
	}
	...
	dcsw_op_all(DCCISW);
#else /* !__aarch64__ */
	assert(xxx_xlat_ctx.xlat_regime == EL1_EL0_REGIME);
	...
	int rc = xlat_change_mem_attributes_ctx(&xxx_xlat_ctx,
				(uintptr_t)xxx_xlat_ctx.tables,
				xxx_xlat_ctx.tables_num * XLAT_TABLE_SIZE,
	...
	if (xxx_xlat_ctx.xlat_regime == EL1_EL0_REGIME) {
		...
		assert(xxx_xlat_ctx.xlat_regime == EL3_REGIME);
		enable_mmu_el3(0U);
	}
#else /* !__aarch64__ */
	...
	if (rc == 0) {
		xxx_xlat_ctx.readonly_tables = true;
	}
...
#define MAX_PHYS_ADDR	xxx_xlat_ctx.pa_max_address
#else
#define MAX_PHYS_ADDR	xxx_xlat_ctx.max_pa
#endif
...
void enable_mmu_el1(unsigned int flags)
{
	setup_mmu_cfg((uint64_t *)&mmu_cfg_params, flags,
		      xxx_xlat_ctx.base_table, MAX_PHYS_ADDR,
		      xxx_xlat_ctx.va_max_address, EL1_EL0_REGIME);
...
void enable_mmu_el2(unsigned int flags)
{
	setup_mmu_cfg((uint64_t *)&mmu_cfg_params, flags,
		      xxx_xlat_ctx.base_table, MAX_PHYS_ADDR,
		      xxx_xlat_ctx.va_max_address, EL2_REGIME);
...
void enable_mmu_el3(unsigned int flags)
{
	setup_mmu_cfg((uint64_t *)&mmu_cfg_params, flags,
		      xxx_xlat_ctx.base_table, MAX_PHYS_ADDR,
		      xxx_xlat_ctx.va_max_address, EL3_REGIME);
