...
class Config:
    ...
    def __init__(self, soc_arch, addrspace_max):
        self.soc_arch = soc_arch
...
class RISCVConfig(Config):
    ...
    def get_device_page_bits(self) -> int:
        ''' Get page size in bits for mapping devices for this arch '''
        if (self.soc_arch == 'riscv32'):
            # 4MiB device pages
            return self.MEGAPAGE_BITS_RV32
        elif (self.soc_arch == 'riscv64'):
            # 2MiB device pages for sv39 and sv48
            return self.MEGAPAGE_BITS_RV64
        raise ValueError('Unsupported soc_arch "{}" specified.'.format(self.soc_arch))


def get_arch_config(soc_arch: str, addrspace_max: int) -> Config:
    ''' Return an appropriate Config object for the given architecture '''
    if soc_arch in ['aarch32', 'aarch64', 'arm_hyp']:
        return ARMConfig(soc_arch, addrspace_max)
    elif soc_arch in ['riscv32', 'riscv64']:
        return RISCVConfig(soc_arch, addrspace_max)
    else:
        raise ValueError('Unsupported soc_arch "{}" specified.'.format(soc_arch))
