...
class FdtParser:
    ...
    def get_kernel_devices(self) -> List[WrappedNode]:
        return self.get_devices_list('neuro,kernel-devices')

    def get_elfloader_devices(self) -> List[WrappedNode]:
        return self.get_devices_list('neuro,elfloader-devices')
    ...
    def get_boot_cpu(self) -> int:
        ''' Returns phandle of the cpu node specified by the neuro,boot-cpu property '''
        chosen = self.get_path('/chosen')
        if not chosen.has_prop('neuro,boot-cpu'):
            raise KeyError('must specify neuro,boot-cpu in /chosen to get boot cpu')

        prop = chosen.get_prop('neuro,boot-cpu')
        return prop.words[0]
